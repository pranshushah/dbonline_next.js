...
function MainGround({
		...
		if (newObj['UNIQUETABLE']) {
			newObj['UNIQUETABLE'].attributes.forEach((id) => {
				newTable.attributes.some((attr) => {
					let ret = false;
					if (attr.id === id) {
						attr.inTableLevelUniquConstraint.push(newObj['UNIQUETABLE'].constraintName);
						ret = true;
					}
					return ret;
				});
			});
			newTable.tableLevelConstraint.UNIQUETABLELEVEL.push(newObj['UNIQUETABLE']);
		}
	...
	function deleteTableModalConfirmHandler() {
		const newTableDndDetails = produce(tableDndDetails, (draft) => {
			const dndIndex = draft.findIndex(
				(table) => table.tableName === selectedTableDndDetailsForDeleteModal.tableName,
			);
			draft.splice(dndIndex, 1);
		});
		const newMainTableDetails = produce(mainTableDetails, (draft) => {
			const mainIndex = draft.findIndex(
				(table) => table.tableName === selectedTableDetailsForDeleteModal.tableName,
			);
	...
	function editTableModalConfirmHandler(newColor, newName) {
		const newTableDndDetails = produce(tableDndDetails, (draft) => {
			const dndIndex = draft.findIndex(
				(table) => table.tableName === selectedTableDndDetailsForEditModal.tableName,
			);
